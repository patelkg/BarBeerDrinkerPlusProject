...
<%String a = request.getParameter("MonthNum");
  String b = request.getParameter("BeerGraph");	
%>
...
String password ="swami100";
	...
	try{
		...
		ArrayList<Map<String,Integer>> list = new ArrayList();
   		...
   		//Make a query
   		String query = "Select Bills.bill_date ,Sum(Transactions.quantity) FROM Bills INNER JOIN Transactions ON Bills.bill_id=Transactions.bill_id where item = '"+b+"' and month(bill_date) = "+a+" Group by Bills.bill_date ORDER BY bill_date , SUM(Transactions.quantity);" ;
   		...
   		while (result.next()) { 
   			map=new HashMap<String,Integer>();
   	   			map.put(result.getDate("Bills.bill_date").toString(),result.getInt("Sum(Transactions.quantity)"));
   			list.add(map);
   	    } 
   	    result.close();
